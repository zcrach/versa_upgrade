...
global max_retry_timeout, image_version, script_is_running, completed_file, image_filename, image_date, image_id
...
def versa_upgrade():
    ...
    try:
        ...
        ch.sendline("cli")
        ch.expect(["cli>", pexpect.EOF, pexpect.TIMEOUT])
        ch.sendline(f"request system package upgrade {image_filename}")
        ch.expect(["[no,yes]", pexpect.EOF, pexpect.TIMEOUT])
        ch.sendline("yes")
        ch.expect(["cli>", pexpect.EOF, pexpect.TIMEOUT])
...
def main():
    ...
    while script_is_running is True:
        ...
        #versa_variables[3] = All statuses are running OK
        print(f"\
        ...
        #If login is successful
            #Check statuses
                #If statuses are OK
                    #Check image file
                        #If image file exists, check version
                            #if version is correct
                            #Shutdown or do some shit
